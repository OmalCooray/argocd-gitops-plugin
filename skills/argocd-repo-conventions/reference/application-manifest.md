...
The manifest above is per-app. The **root** application is different: it creates
the child `Application` objects and nothing else.

- Single `source:` block (not `sources:`), pointing at `path: environments/<env>/apps`
  with `directory: { recurse: false }`.
- `metadata.name: root-<env>`, `metadata.namespace: <argocd-namespace>`.
- `destination.namespace: <argocd-namespace>` — **not** an app namespace; it only
  creates child `Application` objects.
- `syncPolicy.automated` with `prune: true` (deleting an `apps/*.yaml` removes that
  app) and `selfHeal: true`.
- `syncOptions: [CreateNamespace=false]` — deliberately opposite the per-app
  manifest, because the argocd namespace already exists.
...
spec:
  ...
  source:
    repoURL: <gitops-repo-url>
    targetRevision: <branch-or-sha>
    path: environments/<env>/apps
    directory:
      recurse: false
  destination:
    server: <dest-cluster-api>
    namespace: <argocd-namespace>
  syncPolicy:
    automated:
      prune: true
...
Generated by `/argocd-init-repo`; you rarely edit it by hand.
